...
def insert_result(str, acc, nmi,ari):
    g_result_dict[str] = {}
    g_result_dict[str]["acc"] = acc
    g_result_dict[str]["nmi"] = nmi
    g_result_dict[str]["ari"] = ari
...
def eval_other_methods(x, y, names=None):
    ...
    print(ari)
    print(y,y_pred)
    tesla_acc(y, y_pred, trade_res)
    print('=' * 80)
    insert_result(args.dataset + " | GMM clustering on raw data",acc,nmi,ari)
    ...
    tesla_acc(y, y_pred, trade_res)
    print('=' * 80)
    insert_result(args.dataset +" | K-Means clustering on raw data",acc,nmi,ari)
    ...
    tesla_acc(y, y_pred, trade_res)
    print('=' * 80)
    insert_result(args.dataset +" | Spectral clustering on raw data",acc,nmi,ari)
    
    for manifold_learner in ['UMAP','tSNE','isomap']:
        ...
        tesla_acc(y, y_pred, trade_res)
        print('=' * 80)
        insert_result(args.dataset + " | GMM clustering on " +
              str(manifold_learner) + " embedding",acc,nmi,ari)
        ...
        tesla_acc(y, y_pred, trade_res)
        print('=' * 80)
        insert_result(args.dataset + " | K-Means " +
              str(manifold_learner) + " embedding",acc,nmi,ari)
        ...
        tesla_acc(y, y_pred, trade_res)
        print('=' * 80)
        insert_result(args.dataset + " | Spectral Clustering on " +
              str(manifold_learner) + " embedding",acc,nmi,ari)
...
def cluster_manifold_in_embedding(hl, y, label_names=None):
    ...
    for cluster in ['GMM', 'KM','SC']:
        ...
        print(ari)
        print('=' * 80)
        insert_result(args.dataset + " | " + args.manifold_learner +
              " on autoencoded embedding with " + args.cluster + " - N2D",acc,nmi,ari)
        if args.visualize:
...
def tesla_acc(y_true, y_pred, trade_res):
    ...
    print("{}/{}={}".format(good,n_tran,(good/n_tran)))
    print(res)
    return (good/n_tran), res
...
if __name__ == "__main__":
    ...
    optimizer = 'adam'
    from datasets import load_mnist, load_mnist_test, load_usps, load_pendigits, load_fashion, load_har, load_tesla, load_tesla_debug
    ...
    elif args.dataset == 'tesla':
        # x, y, trade_res = load_tesla()
        pass

    result_dict = {}

    # x, y, trade_res = load_tesla(window_size=1, period=0)
    # x, y, trade_res = load_tesla_debug(window_size=32)
    # eval_other_methods(x, y, label_names)

    for window_size in [32]:
        x, y, trade_res = load_tesla(window_size=window_size, period=0)
        # x, y, trade_res = load_tesla_debug(window_size=window_size)
        
        shape = [x.shape[-1], 500, 500, 2000, args.n_clusters]
        
        autoencoderx = autoencoder(shape)

        hidden = autoencoderx.get_layer(name='encoder_%d' % (len(shape) - 2)).output
        encoder = Model(inputs=autoencoderx.input, outputs=hidden)
        ...
        if args.period is None:
            ...
            w_str = 'weights/' +  args.dataset +  "-" + str(args.pretrain_epochs) + "-" + str(window_size) + '-ae_weights.h5'
        else:
            w_str = 'weights/' +  args.dataset +  "-" + str(args.period) + "-" + str(args.pretrain_epochs) + "-" + str(window_size) + '-ae_weights.h5'

        if args.ae_weights is None:
            autoencoderx.compile(loss='mse', optimizer=optimizer)
            autoencoderx.fit(
                x,
                x,
            ...
            # autoencoderx.save_weights(w_str)
            print("Time to train the autoencoder: " + str(pretrain_time))

            # autoencoderx.load_weights(w_str)
        else:
            autoencoderx.load_weights('weights/' + args.ae_weights)
        ...
        # monkey_acc, monkey_res = tesla_acc(y, np.random.randint(2, size=len(y)), trade_res)
        n2d_acc, n2d_res = tesla_acc(y, clusters, trade_res)

        # result_dict[str(window_size)] = {'t_acc':t_acc,'t_nmi':t_nmi,'t_ari':t_ari,'monkey_acc':monkey_acc,'monkey_res':monkey_res,'n2d_acc':n2d_acc,'n2d_res':n2d_res}
    print(result_dict)
